...
module.exports.update_troop = async function (user_id, user_trp_id, x, y, health) {
  try {
    console.log({ user_id, user_trp_id, x, y })
    let sql = `UPDATE user_troops SET troop_x = $3, troop_y = $4 , troop_current_health = $5 WHERE user_id =$1 and user_trp_id =$2; `;
    let result = await pool.query(sql, [user_id, user_trp_id, x, y, health]);
    let troops = result.rows;
    return { status: 200, result: troops };
  } catch (err) {
    console.log(err);
...
module.exports.train = async function (user_id, troop_id, troop_x, troop_y, troop_current_health) {
  try {
    let sql = `Insert into user_troops (user_id,troop_id,troop_x,troop_y,troop_current_health)values ($1,$2,$3,$4,$5) `;
    let result = await pool.query(sql, [user_id, troop_id, troop_x, troop_y, troop_current_health]);
    let troops = result.rows;
    return { status: 200, result: troops };
  } catch (err) {
    console.log(err);
